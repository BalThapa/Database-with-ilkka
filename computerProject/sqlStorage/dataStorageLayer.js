...
const { insertParameters, updateParameters } = require(path.join(
  __dirname,
  config.parameterFunctions
));
      ...
      try {
        if (resource) {
          if (!resource[PRIMARY_KEY]) {
            ...
            const result = await this.db.doQuery(getOneSql, [
              resource[PRIMARY_KEY],
            ]);
            if (result.queryResult.length > 0) {
              reject(
                MESSAGES.ALREADY_IN_USE(PRIMARY_KEY, resource[PRIMARY_KEY])
              );
            } else {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (!id) {
          ...
          if (status.queryResult.rowsChange > 0) {
